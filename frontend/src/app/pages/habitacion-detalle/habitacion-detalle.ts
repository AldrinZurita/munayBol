...
export class HabitacionDetalle implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.loadingService.show('Cargando detalles de la habitación...');
  ...
  getNombreSeccion(): string {
    if (!this.hotel) return ''; // Muestra vacío si el hotel aún no carga
  ...
  getCalificacion(): string {
    if (!this.hotel) return '';
    const scoreInt = Math.round(this.hotel.calificacion || 0);
    return `${scoreInt}/5 (3 reseñas)`;
  }
